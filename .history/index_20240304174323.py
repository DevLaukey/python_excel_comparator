...
def compare_and_visualize(scenario_name, df_operations, df_overall, writer):
    ...
    try:
        ...
        if not df_compare.empty:
            ...
            # Write the comparison results and statistics to Excel sheets
            ...
            df_statistics.to_excel(writer, sheet_name=sheet_name_statistics)
...
def main():
    ...
    # Ensure there is at least one sheet in the Excel file
    with pd.ExcelWriter("output.xlsx", engine='openpyxl') as writer:
        writer.book.create_sheet("Dummy Sheet")  # Add a dummy sheet
        writer.book.remove(writer.book["Dummy Sheet"])  # Remove the dummy sheet
